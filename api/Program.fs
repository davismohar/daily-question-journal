...
let connectionString = "postgres://postgres:postgres@localhost:5432/daily_questions"
...
let getQuestionByMonthAndDay (connection: Sql.SqlProps) (month: int) (day: int): Question option =
    connection
    |> Sql.query "SELECT id, question, month_of_year, day_of_month FROM questions WHERE month_of_year = @month AND day_of_month = @day"
...
let handleGetQuestionWithAnswers (connection: Sql.SqlProps) (questionId: string) (subject: string): HttpHandler =
    ...
    | true, guid ->
        ...
        match question with
        | Some q -> Response.ofJson { id = q.id; question = q.question; day = q.day; month = q.month; answers = answers }
        | None -> Response.withStatusCode 404 >> Response.ofPlainText "Question not found"
    | false, _ -> Response.withStatusCode 400 >> Response.ofPlainText "Invalid ID format"
...
let handleDeleteQuestion (connection: Sql.SqlProps) (id: string): HttpHandler =
    ...
    | false, _ -> 
        Response.withStatusCode 400 >> Response.ofPlainText "Invalid ID format"

let insertAnswer (connection: Sql.SqlProps) (questionId: Guid) (answer: AnswerRequest) (userId: Guid): Answer =
    connection
    |> Sql.query "INSERT INTO answers (question_id, answer, user_id) VALUES (@questionId, @answer, @userId) RETURNING id, answer, created_at"
    |> Sql.parameters [ "questionId", Sql.uuid questionId; "answer", Sql.string answer.answer; "userId", Sql.uuid userId ]
    |> Sql.executeRow (fun read ->
        {
            id = read.uuid "id"
            answer = read.string "answer"
...
let insertUser (connection: Sql.SqlProps) (user: UserRegistrationRequest): User =
    ...
    | None -> // Insert new user if not found
        connection
        |> Sql.query "INSERT INTO users (subject, email, given_name, name, picture, nickname) VALUES (@subject, @email, @given_name, @name, @picture, @nickname) RETURNING id, subject, email, given_name, name, picture, nickname"
        |> Sql.parameters [ "subject", Sql.string user.subject; "email", Sql.string user.email; "given_name", Sql.string user.given_name; "name", Sql.string user.name; "picture", Sql.string user.picture; "nickname", Sql.string user.nickname ]
        |> Sql.executeRow (fun read ->
            {
                id = read.uuid "id"
                subject = read.string "subject"
...
let validateUserRegistrationRequest (user: UserRegistrationRequest): bool =
    user.subject <> "" && user.email <> "" && user.subject <> null && user.email <> null
...
let handlePostUser (connection: Sql.SqlProps) : HttpHandler =
    Request.mapJson (fun user ->
        ...
        if validateUserRegistrationRequest user then
            ...
            Response.withStatusCode 400 >> Response.ofPlainText "Invalid user registration request"
    )
    ...
    webHost args {
        ...
        endpoints [
            ...
            get "/api/questions" (fun ctx ->
                ...
                | Some month, Some day -> Response.ofJson (getQuestionByMonthAndDay (Sql.connect connectionString) month day) ctx
            ...
            get "/api/questions/{id}/answers" (fun ctx ->
                let route = Request.getRoute ctx
                let id = route.GetString "id"
                Request.ifAuthenticated (fun ctx -> handleGetQuestionWithAnswers (Sql.connect connectionString) id ctx.User.Identity.Name ctx) (Response.withStatusCode 401 >> Response.ofPlainText "not authenticated") ctx
            )
            post "/api/questions/{id}/answers" (fun ctx ->
                let route = Request.getRoute ctx
                let id = route.GetString "id"
                Request.ifAuthenticated (fun ctx -> handlePostAnswer (Sql.connect connectionString) id ctx.User.Identity.Name ctx) (Response.withStatusCode 401 >> Response.ofPlainText "not authenticated") ctx
            )
